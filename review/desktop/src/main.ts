...
app.setName('BasicLLM');
app.setPath('userData', path.join(app.getPath('appData'), 'BasicLLM'));
...
if (!app.requestSingleInstanceLock()) {
  ...
  app.whenReady().then(async () => {
    ...
    try {
      ...
      console.log(`[desktop] BasicLLM running on http://127.0.0.1:${port}`);
