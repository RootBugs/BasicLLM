...
Lean Electron menu-bar app for [basicllm](https://github.com/tashfeenahmed/basicllm):
...
npm run desktop:dist:win    # Windows → desktop/dist-electron/BasicLLM-…-x64 installer
```
...
- DB + config live in `~/Library/Application Support/BasicLLM/` (macOS) /
  `%APPDATA%/BasicLLM/` (Windows).
