...
[![CI](https://github.com/tashfeenahmed/basicllm/actions/workflows/ci.yml/badge.svg)](https://github.com/tashfeenahmed/basicllm/actions/workflows/ci.yml)
[![License: MIT](https://img.shields.io/badge/License-MIT-green.svg)](./LICENSE)
[![PRs Welcome](https://img.shields.io/badge/PRs-welcome-brightgreen.svg)](#contributing)
[![Docker image](https://img.shields.io/badge/ghcr.io-basicllm-2496ED?logo=docker&logoColor=white)](https://github.com/tashfeenahmed/basicllm/pkgs/container/basicllm)

**[basicllm.co](https://basicllm.co)** — browse the live model catalog
...
The problem is that stacking them by hand is painful: seventeen different SDKs, seventeen different rate limits, seventeen places a request can fail. BasicLLM collapses that into one OpenAI-compatible endpoint. Point any OpenAI client library at your local server, and it routes transparently across whichever providers you've added keys for.
...
- **Unified API key** — Clients authenticate to your proxy with a single `basicllm-…` bearer token. You never expose upstream provider keys to your apps.
...
**One-liner** (Docker required — sets up `~/basicllm`, generates an encryption key, pulls the image, and starts the container):

```bash
curl -fsSL https://basicllm.co/install.sh | bash
```

Prefer to read before you pipe to bash? [The script is here](https://basicllm.co/install.sh). Re-running it is safe: your `.env` (and encryption key) is preserved and the container updates to `:latest`. Override the defaults with `FREELLMAPI_DIR`, `PORT`, or `HOST_BIND` env vars.
...
```bash
git clone https://github.com/tashfeenahmed/basicllm.git
cd basicllm
...
BasicLLM publishes a single production image that contains the Express server and the built React dashboard:

```bash
docker pull ghcr.io/tashfeenahmed/basicllm:latest   # or pin a release, e.g. :v1.2.3
```
...
```bash
docker compose up -d
docker compose logs -f basicllm
```
...
SQLite data is stored in the `basicllm-data` volume at `/app/server/data`. Keep the same `.env` `ENCRYPTION_KEY` and volume when upgrading, because provider keys are encrypted at rest.
...
![BasicLLM desktop app](repo-assets/desktop.png)

**[Download the macOS app from Releases](https://github.com/tashfeenahmed/basicllm/releases/latest)**, or build it from this repo in a few minutes:

```bash
npm install
npm run desktop:dist        # macOS: desktop/dist-electron/BasicLLM-…-arm64.dmg
...
from [basicllm.co](https://basicllm.co) twice a day and applies new models,
...
- **[Premium](https://basicllm.co/#pricing)** ($19/yr or $49 lifetime) follows
  ...
  [basicllm.co/manage](https://basicllm.co/manage).
...
client = OpenAI(
    ...
    api_key="basicllm-your-unified-key",
)
...
curl http://localhost:3001/v1/chat/completions \
  -H "Authorization: Bearer basicllm-your-unified-key" \
...
When BasicLLM falls over to a different model mid-conversation (quota, rate limit, cooldown), the new model has no idea it is picking up someone else's task. **Context handoff** adds a single compact `system` message to the outbound request that tells the new model exactly that:

```
BasicLLM context handoff:
...
> **Important:** Context Handoff improves continuity for conversations routed through BasicLLM. It cannot recover provider-internal hidden state or messages that were never sent to the proxy.
...
**This project is for personal experimentation and learning, not production.** Free tiers exist so developers can prototype against them; they aren't a stable, supported inference substrate and shouldn't be treated as one. If you build something real on top of BasicLLM, swap in a paid API before you ship. Your relationship with each upstream provider is governed by the terms you accepted when you created your account — those terms still apply when the traffic is proxied through this project, and you're responsible for complying with them.

## Star History

[![Star History Chart](https://api.star-history.com/chart?repos=tashfeenahmed/basicllm&type=date&legend=top-left)](https://www.star-history.com/?repos=tashfeenahmed%2Fbasicllm&type=date&legend=top-left)
